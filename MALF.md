...
| `contract-gen` | `malf contract-gen [DIR|FILE...] [--out PATH]` renders the DERIVED contract surface (ADR-26.O3) from the tagged comment lines and law blocks the format gate counts: per declaration its `pre`/`post`/`invariant` and `refs:`, every law block in full, the refs index and the `TEST` → `refs:` table. Stdout by default; `--out` writes a scratch file. Never committed — regenerate it. `--selftest` proves the renderer on a fixture. |
