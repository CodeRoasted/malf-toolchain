...
| `commands` / `compile-commands` | configure every member package (+ its `test_package`) and merge all `compile_commands.json` into the repo root's default-leg DB. **Every run states its own scope** — members selected/configured, the entry total split first-party vs external, and per member its entries and how many are test/bench TUs — and a member left in **dependency role**, or one that never configured, is **fatal** rather than a smaller number |
