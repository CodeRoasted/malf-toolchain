...
| `lint` | clang-tidy on git-changed files (or `--all-files`). Requires a compile database — it refuses to run without one, because a flag-less clang-tidy reports phantom errors it cannot be right about. Under `--all-files` an empty file set is a scoping failure, not a pass. A TU clang-tidy **dies** on is reported as **NOT LINTED** by name, apart from the findings, and fails the run: a crash dump carries real paths and real line numbers, so merged into the findings it reads as a source defect while the file's zero coverage leaves no trace. It also DE-SYSTEMS first-party include roots before handing the database to clang-tidy: CMake marks an IMPORTED target's include directories SYSTEM, so our own editable packages arrive as `-isystem` and the checker reports nothing about them; the run rewrites those to `-I` in a derived database and prints what it rewrote and what it left. Third-party stays `-isystem`. **Every run ends with a one-line `SUMMARY`** naming the selection mode (`git diff --name-only HEAD` vs `--all-files`), the directory, the population the mode selected, how many of those actually reached clang-tidy, and the finding / not-linted counts — because a green that does not state its scope reads the same whether it checked 400 files or none. A run that selected **zero** files says `CHECKED 0 — NOTHING WAS INSPECTED` and states in words that it is not a clean verdict. Under `--all-files` a TU with no compile command is a fatal coverage hole **unless the build gates it off this platform**: `platform_gate.py` finds every CMake line of the repo naming the file and, when each one sits in an `if()` branch built only of platform words (`WIN32`, `UNIX`, `APPLE`, `LINUX`, their `CMAKE_HOST_` forms, `NOT`/`AND`/`OR`) that `cmake -P` does not take on this host, the TU is **REFUSED** — named with the CMake line and branch, counted as `platform-refused` in the `SUMMARY`, and not fatal. A file no CMake line names, one under a project option, or one under a branch that IS taken here stays fatal; the verdict never reads a file name |
| `format` | clang-format all C++ files in-place (`--check` = `--dry-run --Werror`, non-zero on any violation). Resolves the style explicitly and refuses to run if it cannot, rather than let clang-format fall back to LLVM style unannounced. **Every run ends with a one-line `SUMMARY`** in the same vocabulary as `lint`'s, naming what the run did and where its population came from (`check`/`write` × `sweep`/`paths`), the directory, `selected` (every file the walk matched), `checked` (those handed to clang-format), the misformatted count in check mode, and `skipped` (the `MALF_SOURCE_MAX_FILE_KB` overruns) — so `selected = checked + skipped` is an identity on the line itself and a non-zero `skipped` is a visible coverage gap. A run that selected **zero** files says `CHECKED 0 — NOTHING WAS INSPECTED` and states in words that it is not a clean verdict, replacing `nothing to do`, which read as a pass. `misformatted` counts FILES, not diagnostics; the write mode reports no such count, because `clang-format -i` never says what it changed. The exit status is `xargs`'s 123 on any failing batch, so it is printed beside the count: rc non-zero with **zero** violations means clang-format failed for a reason that is not formatting, and those files were not checked at all. **Then a second phase, over the SAME population: the Code & Comment as Contract grammar** (`comment_contract_lint.py`, `ADR-26.D7`) — every `//` line in every C++ repo begins with one of `pre:` `post:` `invariant:` `assert:` `note:` `refs:` on its own line (a contract form may carry ONE untagged continuation line; a note and a refs never), the only multi-line comment is the framed `D-LSRC-n` law block, and the tool forms a machine reads (`} // namespace x`, `/*name=*/`, own-line `NOLINT…` preceded by its `note:`/`refs:`, `clang-format off/on`) are admitted. It reads the **post-format** text — in check mode it formats each file to stdout itself — because the reflow swallows a `post:` tag into an overlong `pre:` line (measured 2026-09-05, clang-format 21.1.8) and a pre-format read cannot see it. The phase reds in every repo with no declaration (since `OPS-8.S13`, 2026-09-09 — the per-repo `comment_contract` key that made an unmigrated repo report-only was ripped when the last migration closed); every violation is printed with its site, and the `CCC SUMMARY` line carries files checked, comment lines and the by-form counts so a zero is distinguishable from nothing inspected. `refs:` addresses are checked for **form** here and for **resolution** by `scripts/registry_grammar_lint.py`, never both in one place |
| `contract-gen` | `malf contract-gen [DIR|FILE...] [--out PATH]` renders the DERIVED contract surface (ADR-26.O3) from the tagged comment lines and law blocks the format gate counts: per declaration every tagged line it carries (`pre`/`post`/`invariant`/`assert`/`note`) and its `refs:`, a `TEST` under its name, every law block in full, the refs index and the `TEST` → `refs:` table. Stdout by default; `--out` writes a scratch file; `--declarations-only` is the narrower shape. Never committed — regenerate it. `--selftest` proves the renderer on a fixture. |
...
**A sweep settles the database whole.** When `build`/`test`/`bench` sweep several members, each member's dependency bootstrap re-configures the earlier members it requires with tests OFF — and where a repo has a root recipe (`insight-eidos`, `coderoast-ipc`) that root package's tree **is** the repo database's address, so each re-configure overwrote the file. Measured on `insight-eidos` 2026-09-21: the CI invocation (`malf build --profile linux-clang21-libcxx-release`) left **84 entries covering 79 of 244 tracked TUs**. So after the last member a sweep re-configures (and builds) every member left in dependency role, plus the repo-root member unconditionally, reads each role back from its `CMakeCache.txt` (still dependency → exit 1), and **recomposes** each repo database from the member trees — never accumulated, so an entry for a deleted TU does not survive a sweep that covers the repo. After it: **243 entries covering 241 of 244**, the 3 absent being the two platform-gated TUs below and `test_package/test_package.cpp`, which only `commands` configures. A single-package build (`--only`, or a leaf) still merges incrementally, and a dependency rebuild of a root recipe from another repo still truncates that repo's database until its next sweep — `lint --all-files` names that as a coverage hole.
